...
class MyProxyMiddleWare():
    '''
    1. 先检查本地IP文件: ipPool.pkl（是否存在，是否超时）
    2. 如果ipPool.pkl里的ip可用则使用，不可用则重新获取并保存
    ...
    def __init__(self) -> None:
        self.ipPool = {'timestamp':None,'ipPool':None}
        self.getIpPool()
    ...
    def getRandomProxy(self):
        random.shuffle(self.ipPool['ipPool'])
        return self.ipPool['ipPool'].pop()
